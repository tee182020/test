...
<CodeLabLayout title="PythonConditionalStatements">
...
#input() function gets the entered number as a string type from user so int() converts it to an integer
...
# Python power operator ** raises the left number to the power of the right numer
if answer == 2**8:
    print("Correct")
else:
...
Python probvides a shorthand method like the ternary operator of C languge for writing if-else statement in one line.
